...
<meta property="product:availability" content="{{ $product->availabilityForFeed() }}">
@endpush
@push('dataLayer')
<script>window.dataLayer.push(@js(['event' => 'view_item', 'ecommerce' => ['currency' => $product->currency, 'value' => (float) $product->price, 'items' => [['item_id' => $product->sku ?: (string) $product->id, 'item_name' => $product->name, 'item_brand' => $product->brand, 'item_category' => $product->category->name, 'price' => (float) $product->price, 'quantity' => 1]]]]));</script>
@endpush
@section('content')
    ...
    <div class="container-shell">
        ...
        <div class="product-detail-grid">
            ...
            <article class="product-detail-copy">
                ...
                <div class="product-detail-price"><strong>{{ number_format((float) $product->price, 2) }} <small>ر.س</small></strong>@if($product->compare_at_price)<del>{{ number_format((float) $product->compare_at_price, 2) }} ر.س</del>@endif</div>
                <span @class(['stock-state', 'is-available' => $product->isAvailable(), 'is-unavailable' => ! $product->isAvailable()])>{{ $product->isAvailable() ? 'متوفر للطلب' : 'نفد المخزون' }}</span>
                ...
                @endif
                <dl class="product-facts">@if($product->sku)<div><dt>رمز المنتج</dt><dd dir="ltr">{{ $product->sku }}</dd></div>@endif<div><dt>العلامة التجارية</dt><dd>{{ $product->brand }}</dd></div><div><dt>الدفع</dt><dd>تحويل بنكي فقط</dd></div></dl>
            </article>
        </div>
